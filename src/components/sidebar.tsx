...
export function Sidebar({
  ...
  const saveNotebookName = async () => {
    if (editingNotebook && editingName.trim()) {
      ...
      const request = { name: editingName.trim() };
      await axios.put<BaseResponse<Update>>(
        `${AppConfig.apiBaseUrl}/api/notebook/v1/${editingNotebook}`,
        request,
      );
  ...
  const handleDrop = (
    ...
    const [draggedType, draggedId] = data.split(":") as [
      "notebook" | "note",
      string,
    ];
  ...
  const renderNotebook = (notebook: Notebook, level = 0) => {
    ...
    const isDragOver =
      dragOverItem?.type === "notebook" && dragOverItem.id === notebook.id;
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  } // Disable dropdown if any operation is processing
                >
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  ...
                  disabled={
                    ...
                    isProcessingMove ||
                    isThisNotebookDeleting
                  } // Disable if already saving or other operations
                >
